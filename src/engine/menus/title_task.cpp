...
static_assert(offsetof(TitleTask_t, languageCursor) == 0x114);
...
u32 TitleTask::LanguageCursor() const {
  const auto *self = Self<TitleTask_t>();
  return self ? static_cast<u32>(self->languageCursor) : 0;
}

void TitleTask::SetLanguageCursor(u32 cursor) {
  auto *self = Self<TitleTask_t>();
  if (self)
    self->languageCursor = cursor;
}

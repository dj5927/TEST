 ...
 *            Additional Android/Korean import work (c) 2026 re:Blue Android contributors.
...
bool HasKorean(rex::filesystem::Entry &root) {
  const auto langs = ParseDiscLanguages(root);
  return langs.contains("kr");
}
...
std::vector<u8> ReadPrefix(rex::filesystem::Entry &entry, size_t wanted) {
  ...
  size_t read = 0;
  file->ReadSync(std::span<u8>(out.data(), out.size()), 0, &read);
  file->Destroy();
  if (read != count)
    return {};
  return out;
...
bool PatchBootIni(const fs::path &game_root, InstallProgress &progress) {
  ...
  while (std::getline(in, line)) {
    ...
    if (line.starts_with("[Language]")) {
      ...
      voice = true;
      // The converted regional SFD topology is C1=JP, C2=KR, C3=JP.
      // Keeping the retail JP/KR order makes Korean voice option 2.
      line = "[Voice]\tJP KR";
    }
...
bool ExtractKoreanText(rex::filesystem::Entry &root, const fs::path &install_root,
                       const fs::path &temp_root, InstallProgress &progress) {
  struct ArchiveSpec {
  ...
  for (const auto &spec : specs) {
    auto *entry = root.ResolvePath(spec.disc_path);
    if (!entry)
  ...
  const std::string mod_toml =
      ...
      "author = \"re:Blue Android contributors\"\n"
      "version = \"1.0\"\n"
...
bool ValidateKoreanRetailSources(
  ...
  for (int i = 0; i < kDiscCount; ++i) {
    auto image = DiscImage::Open(sources[i]);
    if (!image || !image->Root(i + 1)) {
      if (error)
        *error = "Could not open Korean retail DVD " + std::to_string(i + 1);
      return false;
    }
    if (!HasKorean(*image->Root(i + 1))) {
      if (error)
...
bool ImportKoreanRetailData(
  ...
  for (size_t di = 0; di < roots.size(); ++di) {
    auto *root = roots[di];
    if (auto *entry = root->ResolvePath("pack/packmem_kr.ipk"); entry &&
    ...
    for (const char *dir_name : {"snd_memory_kr", "snd_stream_kr"}) {
      auto *dir = root->ResolvePath(dir_name);
  ...
  if (!ExtractKoreanText(*roots[0], install_root, temp_root, progress)) {
    fs::remove_all(temp_root, ec);
    return false;
  }
  ...
  for (auto *root : roots) {
    auto *movie_dir = root->ResolvePath("movie");
    if (!movie_dir)
      continue;
    std::vector<DiscFile> found;
  ...
  // 4) Expose KR in the base boot configuration while preserving the
  // validated JP/KR voice order required by the converted movies.

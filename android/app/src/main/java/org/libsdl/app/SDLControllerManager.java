        ...
        for (int device_id : deviceIds) {
            if (SDLControllerManager.isDeviceSDLJoystick(device_id)) {
                ...
                if (joystick == null) {
                    ...
                    // Some Android handhelds expose the built-in controller motors only
                    // through InputDevice.getVibrator(), while getVibratorManager() reports
                    // no vibrator IDs. Accept either API so Retroid/Odin-style integrated
                    // gamepads aren't incorrectly marked as non-rumble devices.
                    try {
                        Vibrator legacyVibrator = joystickDevice.getVibrator();
                        can_rumble = (legacyVibrator != null && legacyVibrator.hasVibrator());
...
class SDLHapticHandler_API31 extends SDLHapticHandler {
    ...
    void rumble(int device_id, float low_frequency_intensity, float high_frequency_intensity, int length) {
        ...
        if (vibrators.length >= 2) {
            ...
            // Vendor handheld firmware may implement the legacy per-InputDevice
            // vibrator without publishing it through VibratorManager.
            try {
                Vibrator legacyVibrator = device.getVibrator();
                if (legacyVibrator != null && legacyVibrator.hasVibrator()) {
                    float intensity = (low_frequency_intensity * 0.6f) + (high_frequency_intensity * 0.4f);
                    vibrate(legacyVibrator, intensity, length);
                }
            } catch (Exception ignored) {
            }
        }
